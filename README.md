...
- **Appending to a running queue now asks first.** While a run was in progress, choosing "Add to task queue" pushed the new items straight into the live worker queue: they started encoding immediately and replaced their originals, with no confirmation beyond the item count and file names. If backup was off, those originals were unrecoverable and nothing said so. Appending during a run now opens its own confirmation dialog that lists the new files, states that they will be compressed at once and overwrite their originals, and reports the actual backup state of this run — with an explicit "cannot be recovered" warning when backup is not effective. Cancelling leaves the running queue untouched.
- **Safety warnings are localized.** The overwrite, backup and unrecoverable warnings in the start-of-run confirmation were hard-coded Simplified Chinese, so users on English, Japanese or any other locale never saw them. Every warning now comes from the locale files, and all eight languages carry a full translation.
- **"Sync back to the Eagle library" no longer reads as an overwrite switch.** The label and the docs implied the original was only replaced when that option was on. It never controlled overwriting — the file at the original path is replaced either way, and the option only decides whether Eagle's replace API updates the linked item and refreshes its thumbnail. The checkbox now has an inline hint, the confirmation dialog says so explicitly, and every language's usage doc has a dedicated "file replacement" section.
- **The action bar buttons stop jumping sideways.** As soon as the summary figures went from "—" to real sizes, "Start compression" and "Stop and cancel" snapped from the left edge to the right. The bar is a wrapping flex row: in a narrow window those buttons drop to a second line while the spacer meant to push them right stays on the first, so nothing pushes them and they sit flush left. The buttons now carry `margin-left: auto` themselves, and auto margins resolve per flex line, so they stay right whichever line they land on.
- **The `hidden` attribute works again.** `display: none` for `[hidden]` comes from the UA stylesheet and loses to any author rule that sets `display`. Both `.notice-row` and `.sample-analysis-controls` set one, so `el.hidden = true` did nothing for them — the "Analyse all / Stop analysis" controls were visible the moment the plugin opened. A `[hidden] { display: none !important }` guard now backs the attribute up.
- **"Open log folder" opens the right folder on Windows.** The fallback branch spawned `explorer` with `['/select,', p]`, so `/select,` and the path arrived as two separate arguments. Explorer does not recognise that as a reveal request and quietly opens Documents instead — with a normal exit code, so the plugin still reported "Log folder opened". The two now form a single argument.
- **Logs and settings no longer diverge on Windows.** The settings file honoured `%APPDATA%` while the log path hard-coded `AppData\Roaming`. On a machine with a redirected `APPDATA` the two ended up in different places, which made diagnostics impossible to line up. The log path now reads `%APPDATA%` the same way.

**Improved**

- **The package now ships only what runs.** Packaging used a deny-list that never applied, so `tests/`, `tools/`, `reports/` and submission notes rode along into the plugin archive. `tools/build-package.sh` replaces it with an allow-list that stages a clean `dist/<id>/` directory, and `tools/verify.sh` fails on any top-level entry outside that list. Nothing new gets in unless it is registered. `--zip` also audits the finished archive: zip integrity, absolute paths and `..`, `__MACOSX`/`._*`/`.DS_Store`, duplicate entries and symlinks. Any hit fails the build.
- **What the plugin leaves on disk is now stated.** Settings and the runtime log survive an uninstall, and users had no way to find out where they live. The log panel now lists the full path of both files, and all eight usage docs gained a "data the plugin keeps" section covering the macOS and Windows locations, the fact that "Reset settings" does not delete the file, and how to clear everything by hand.
- **The log panel is no longer Chinese-only.** Toasts such as "Copied to clipboard" and "Could not open it automatically" were hard-coded and showed up in Chinese on every locale. They now come from the locale files. This also fixed toasts overwriting each other: dismissing a toast used to restore the text captured before it appeared, so two clicks within 1.8 seconds restored the *previous toast* and the paths were replaced by a message for good.
- **Store copy has a source of truth.** The name and description submitted to the plugin store lived only in the submission form, where the length limits were discovered after rejection. They now live in `store/descriptions.json`, and `tools/check-store.js` enforces the per-locale character limits, matching plugin names, and unterminated sentences before submission.
- **Usage docs state the FFmpeg requirement.** All eight docs now open the usage section with a "before you start" block covering the FFmpeg dependency plugin, the local FFmpeg/ffprobe fallback, and what the plugin does when neither is found.
